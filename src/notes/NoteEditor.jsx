...
export default function NoteEditor ({ title, content, saveNote, destroyNote, helia }) {
  ...
  const publishNote = async () => {
    ...
    const j = json(helia)
    const cid = await j.add(stuff)
    setIpfsCid(cid.toString())
    // make a request to ipfs.io and a couple gateways to get them to cache the file
    // dont await we dont care about the result
    axios.get("https://ipfs.io/ipfs/"+cid.toString())
    axios.get("https://gateway.pinata.cloud/ipfs/"+cid.toString())
  }
  ...
  return (<div className="flex flex-col flex-1">
    ...
    <div className="flex flex-row flex-1 my-4 align-center">
        ...
        <button className="btn btn-sm btn-ghost ml-2" onClick={async () => { await navigator.clipboard.writeText(ipfsCid) }}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
